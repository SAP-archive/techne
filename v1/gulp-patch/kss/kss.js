...
var walk = require('./walk.js'),
	...
	isDeprecated, isExperimental, hasPrefix, getMarkdownNav,
	commentExpressions = {
		single: /\s*?\/\/(.*?)$/g,
		multiStart: /\/\*\!?(.*?)$/,
		multiFinish: /\*\//,
...
getMarkdownNav = function(file){
    var markDownNav = undefined;
    var commentBlock = file.contents.toString('utf-8').match(/<!--SIDENAVCONFIG[\s\S]*?-->/g);
    
    if( commentBlock ){
        markDownNav = JSON.parse(commentBlock.toString().split("<!--SIDENAVCONFIG")[1].split('-->')[0]);
    }
